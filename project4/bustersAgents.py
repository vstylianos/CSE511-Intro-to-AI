...
class GreedyBustersAgent(BustersAgent):
  ...
  def chooseAction(self, gameState):
    ...
    livingGhostPositionDistributions = [beliefs for i,beliefs
                                        ...
                                        if livingGhosts[i+1]]
    "*** YOUR CODE HERE ***"
    likelihood_pos = [(belief.argMax(), belief[belief.argMax()]) \
                              for belief in livingGhostPositionDistributions]
    probable_ghost_pos, _ = max(likelihood_pos, key = lambda x: x[1])
        
    action_distr = lambda action: self.distancer.getDistance(
                                            Actions.getSuccessor(pacmanPosition, action),
                                            probable_ghost_pos)

    action = min(legal, key = action_distr)
    return action
