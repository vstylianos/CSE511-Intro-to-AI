...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        self.costFn = lambda a: 1 #define the cost function in lambda notation!
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if nextState == self.corners[3]:
                    x4=True
                nextState = ((nextx, nexty),x1,x2,x3,x4)
                cost = self.costFn(nextState)
                successors.append( ( nextState, action, cost) )
...
def cornersHeuristic(state, problem):
    ...
    for i in range(0,4):
        if xy[i+1] == False:
            temp = max(0,mazeDistance(xy[0],corners[i], problem.startingGameState))
            unvist_corn[i] = temp 
            
    
    return max(unvist_corn)
...
def foodHeuristic(state, problem):
    ...
    for food in pellets:
        z = mazeDistance(position, food, problem.startingGameState)  
